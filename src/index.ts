...
async function fetchTravelBuddy(url: string): Promise<string> {
  try {
...
server.tool(
  "get_travel_weather",
  "Get weather for a destination and date range (Travel Buddy API). Use for trip planning.",
  {
...
server.tool(
  "search_hotels",
  "Search hotels by location and stay dates (Travel Buddy API).",
  {
...
server.tool(
  "search_flights",
  "Search flights by origin, destination airport/city codes, and departure date (Travel Buddy API).",
  {
    origin: z
      .string()
      .min(2)
...
server.tool(
  "get_maps_context",
  "Get map context for multiple locations on a route or trip (Travel Buddy API).",
  {
    locations: z
      .string()
      .min(1)
      .describe(
        "Comma-separated place names (e.g. Mumbai,Amritsar,Wagah_Border)",
      ),
  },
  async ({ locations }) => {
...
server.tool(
  "get_travel_content",
  "Get travel content for a location (Travel Buddy API).",
  {
